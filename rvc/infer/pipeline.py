...
class Pipeline:
    ...
    def voice_conversion(
        ...
        Args:
            model: The feature extractor model. (hubert)
        ...
        """
        print("padded audio frames")
        print(len(audio0))
        
        print("padded audio length in seconds")
        print(len(audio0)/16000)
        ...
        with torch.no_grad():
            ...
            print("hubert outputs for every 20ms")
            print(feats.shape)
            mean_distance = []
            ...
            if (
                ...
                feats, mean_distance = self._retrieve_speaker_embeddings(
    ...
    def _retrieve_speaker_embeddings(self, feats, index, big_npy, index_rate):
        ...
        closest_feature_distance = score.min(axis=1).sum()

        weight = np.square(1 / score)
        weight /= weight.sum(axis=1, keepdims=True) # normalization every 8 values in row sum to 1
        ...
        mean_distance = closest_feature_distance/len(feats)
        return feats, mean_distance
    ...
    def pipeline(
        ...
        # opt_ts optimal timestamps
        # following algorithm inteligently splits audio in most quiet places
        opt_ts = []
        if audio_pad.shape[0] > self.t_max: # if is audio longer that maximum procesing lenght
        ...
        audio_opt = []
        t = None
        audio_pad = np.pad(audio, (self.t_pad, self.t_pad), mode="reflect") # t_pad is 1 or 3 seconds depending on GPU
        ...
        for t in opt_ts:
            ...
            if pitch_guidance:
                aud, mean_segment_feature_distance = self.voice_conversion(
                    model,
                    net_g,
                    sid,
                ...
                mean_distances.append(mean_segment_feature_distance)
                audio_opt.append(aud[self.t_pad_tgt : -self.t_pad_tgt])
            else:
                aud, mean_segment_feature_distance = self.voice_conversion(
                    model,
                    net_g,
        ...
        mean_feature_distances = np.sum(mean_distances)/len(mean_distances)
        print("mean_feature_distances", mean_feature_distances)
